...
def run(arg1,arg2,arg3,arg4,arg5):
    ...
    Secret = 'mCaP57nCwC' # <-- 固定的 APP Secret
    apk = ''
    ...
    try:
        ...
        messages.append(f"【{mark}】登录失败: 网络连接异常或DNS错误: {e}")
        print(f"❌登录失败: 网络连接异常或DNS错误: {e}")
        return

    if "登录成功" not in response.get("message",""):
        messages.append(f"【{mark}】登陆失败: 账密错误或封号")
        print(f"⭕登陆：账密错误")
        return
        
    auth = response["data"]["token"] # <-- 登录成功后获取 auth
            ...
            try:
                ...
                if thisturn == 7 and arg4:
                    ...
                    if responseday["data"]["boxdata"]['type'] == "money":
                        ...
                        messages.append(f"⭕连签红包：{responseday['message']}")
...
def main():
    ...
    for i, ck_run_n in enumerate(ck_run):
        ...
        if len(parts) == 3:
            ...
            print(f"⭕当前账号：{mark} - 格式错误，APP模式应为：备注#手机号#密码，跳过！")
            continue
